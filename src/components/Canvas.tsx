...
import { Component, Wire, Point, CurrentFlow, ComponentTemplate } from '../types/ComponentTypes';
  ...
  useEffect(() => {
    ...
    const animationInterval = setInterval(() => {
      setCurrentFlows(prevFlows => 
        prevFlows.map(flow => ({
          ...flow,
          position: (flow.position + 0.01 * flow.direction) % 1
        }))
      );
    }, 50);
